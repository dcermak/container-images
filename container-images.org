...
#+REVEAL_TITLE_SLIDE: <div style="float:left"><a href="https://www.linuxdays.cz/2026/" target="_blank"><img src="./media/LinuxDays.svg" height="100px"/></a></div>
...
@@html: <tr><td><i class="fab fa-suse"></i></td><td> Open Source Advocate @SUSE</td></tr>@@
...
- [[Software Delivery: The Real Problem][Containers from Scratch]]
...
#+begin_notes
- devs typically run "bleeding edge", production is a "stable" distribution
  \Rightarrow version conflicts are guaranteed, especially subtle ones
- debugging prod is "fun", so can't we ship "dev"?
- often subtle bugs due to some obscure library or what not
- so a VM? \rightarrow has the issue of a hypervisor and developing there sucks too
- just a tarball? \rightarrow simple, but awful UX
#+end_notes

#+ATTR_REVEAL: :frag (appear)
- Dev environment != Production environment
- Deploy \Rightarrow 3h downtime
- VM?
- Tarball?
...
#+begin_notes
- first idea: if the problem is environment differences, bundle the whole environment
- chroot creates isolated filesystem view
#+end_notes
...
#+begin_notes
- manual \rightarrow bad
- huge archives are unwieldy (app + OS)
- build process inefficient
- no isolation of processes
- no resource restrictions
- network uncontrolled/unrestricted
#+end_notes

#+ATTR_REVEAL: :frag (appear)
@@html:<img src="./media/conflicting-apps.svg"/>@@

** Part 2: Process Isolation

#+begin_notes
- kernel can create "barriers" between processes & restrict their access to resources
- every major OS has this one way or another
#+end_notes
...
#+ATTR_REVEAL: :frag (appear)
@@html:<img src="./media/process-isolation.svg"/>@@

** Linux Namespaces

#+begin_notes
- Namespaces provide process isolation \rightarrow what containers need
- form a tree and inherit parents restrictions
...
#+begin_notes
- isolation in place, but no protection from resource consumption:
  could eat all the RAM, consume too much CPU, no monitoring, no disk I/O limits, etc.
- cgroups provide the missing piece - resource control and limits
- started in 2006, merged in 2008 (2.6.24)
- redesigned to v2 in 2016 (4.5)

- resource limits like I/O, FS caches, CPU quota, open files
- resource prioritization between groups
- measure whole group resource usage
- freeze, restart, kill groups

- nowadays used by userspace memory killers
- modern DEs put each process into a cgroup \rightarrow for oom-killers

\rightarrow see cgroup-demo.sh
#+end_notes

#+ATTR_REVEAL: :frag (appear)
@@html:<img src="./media/cgroups.svg"/>@@


** Part 4: Syscall filtering

#+begin_notes
- there's a ton of syscalls, some of them can be used in a dangerous fashion, like =kexec_load=
- applications generally don't need access to all the syscalls available
- on Linux, syscall filtering can be done via =seccomp=
- seccomp rules survive =execve=
- optional demo, see seccomp-demo.sh
- docker's seccomp rules: https://docs.docker.com/engine/security/seccomp
#+end_notes
#+ATTR_REVEAL: :frag (appear)
@@html:<img src="./media/seccomp.svg"/>@@

** Part 5: Capabilities

#+begin_notes
- more granular setting of what each process can do, versus the original Unix
  style where root does everything
- certain overlap to seccomp, but seccomp is purely syscall level, this is on a
  workflow level (i.e. can I call =execve= vs can I bind to ports)

- view current capabilities: =capsh --print= or =grep ^Cap /proc/self/status= (the
  later is just a bitset)
...
- some capabilities:
  ...
  =CAP_KILL=: signal processes you normally couldn't
  =CAP_BPF=: Employ privileged BPF operations
  =CAP_SYS_ADMIN=: a very broad collection of admin operations

- different types:
  * permitted: capabilities the process is allowed to use
  * effective: capabilities currently active for permission checks
  * bounding: ceiling on capabilities the process can acquire
  * ambient: capabilities that can survive =execve()=

- fun fact: files have capabilities too: =getcap /path/to/file=, when invoked, the
  new process will have the added capability
  Capabilities can only be added by a process with =CAP_SETFCAP=

- =setcap=/=getcap= report capabilities as ~$CAP{+,-,=}eip~, where e = Effective
  (auto enabled), i = Inheritable (survives =execve=, but not granted!), p =
  Permitted (can be acquired, but not enabled by default)
  There's also =+= and =-= for capabilities, besides ~=~ (~=~ assigns, the others add or take away)

- important: ~int prctl(PR_SET_NO_NEW_PRIVS, 1L, 0L, 0L, 0L);~
  no new privileges can be acquired \Rightarrow important for containers to prevent setuid exploit

see also =man 7 capabilities=
#+end_notes
...
#+begin_notes
- Docker directly addresses each of the pain points
- show how Docker features map to manual problems
- this is the payoff moment - all the technical details come together
#+end_notes
...
#+begin_notes
- usually build is a multi-step process \Rightarrow how about caching?
- also, launching five containers *must* not result in five full copies
#+end_notes

#+ATTR_REVEAL: :frag (appear) :code_attribs data-line-numbers='2|3|4|5|'
...
#+begin_notes
- PID 1 signal handling model works well
#+end_notes
...
#+ATTR_REVEAL: :frag (appear)
1. Do I have environment consistency problems?
2. Is my app a single process that I can isolate?
3. Do I need to share/distribute my app environment?
4. Can I separate data & code in deployment?
5. Am I willing to learn new deployment patterns?
